...
reset_observation_vector = np.zeros((6,t_dim[0],t_dim[1]), dtype=np.float32)
...
class RCCarEnv(py_environment.PyEnvironment):
    ...
    def __init__(self):
        ...
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(6,t_dim[0],t_dim[1]), dtype=np.float32, minimum=0, maximum=500.0, name='observation')
    ...
    def _step(self, next_action):
        ...
        if self._episode_ended or self._observation[4, 0, 0] > .85:
            ...
            # mock_t = np.array([float(i) for i in range(t_dim[0]*t_dim[1])], dtype=np.float32).reshape(t_dim)
            # mock_d = np.array([100, 100, 25, 25], dtype=np.float32)

            # Generate pipeline for next step
            new_pipeline = Pipeline(mock_t, mock_d, prev_t_percent=self._observation[4], prev_d_score=prev_d_score, width=t_dim[1], height=t_dim[0])
            ...
            new_observation_vector = np.zeros((6,t_dim[0],t_dim[1]), dtype=np.float32)
